...
import com.cloudtimes.common.utils.ip.AddressUtils;
...
public class AppTokenService {
    ...
    private static Logger log = LoggerFactory.getLogger(AppTokenService.class);
    ...
    private void checkIsSingleLogin(AppLoginUser newLoginUserToken) {

        String isSignleLogin = configService.selectConfigByKey("member.singleLogin");
        if (StringUtils.isBlank(isSignleLogin) || !isSignleLogin.equals("1")) {
            return;
        }

        Collection<String> keys = redisCache.keys(CacheConstants.APP_LOGIN_TOKEN_KEY + "*");
        List<AppLoginUser> userOnlineList = new ArrayList<>();
        for (String key : keys) {
            AppLoginUser user = redisCache.getCacheObject(key);
            if (StringUtils.isNotNull(user)) {
                if (newLoginUserToken.getUsername().equals(user.getUsername()) && !newLoginUserToken.getToken().equals(user.getToken())) {
                    userOnlineList.add(user);
                }
            }
        }

        for (AppLoginUser appLoginUser : userOnlineList) {
            redisCache.deleteObject(CacheConstants.APP_LOGIN_TOKEN_KEY + appLoginUser.getToken());
            log.info(appLoginUser.getUsername() + "的登录令牌[" + appLoginUser.getToken() + "]已设置为失败!");
        }
    }
}
