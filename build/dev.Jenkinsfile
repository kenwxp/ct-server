...
pipeline {
    ...
    stages {
        ...
        stage('Replace Env') {
            steps {
                sh 'bash ./build/replace_env.sh dev'
            }
        }
        ...
        stage("Prepare Deploy") {
            steps {
                ...
                sshPut remote: dev233, from: './build/check_all.sh', into: "$REMOTE_WORK_DIR/"
        ...
        stage('Restart') {
            steps {
                ...
                sleep 10
                sshCommand remote: dev233, command: "bash $REMOTE_WORK_DIR/check_all.sh"
            }
        }
    }
}